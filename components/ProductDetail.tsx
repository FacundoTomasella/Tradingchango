...
const ProductDetail: React.FC<ProductDetailProps> = ({ productId, onClose, onFavoriteToggle, isFavorite, products, theme }) => {
  ...
  // Trading logic: Increase in price is RED (bad for consumer), Decrease is GREEN (good)
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col md:items-center md:justify-center bg-black/80 md:backdrop-blur-sm transition-all duration-300">
      <div 
        ref={modalRef}
        className="w-full md:max-w-xl h-full md:h-auto md:max-h-[96vh] bg-white dark:bg-black md:rounded-[2rem] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom-10"
      >
        {/* Detail Header - EXACT AS SCREENSHOT */}
        <div className="p-4 flex justify-between items-center bg-white dark:bg-black border-b border-slate-100 dark:border-[#121212]">
          <div className="flex items-center gap-4">
            <button onClick={onClose} className="text-black dark:text-white p-2 -m-2">
              <i className="fa-solid fa-arrow-left text-xl"></i>
            </button>
            <span className="font-[800] text-[17px] tracking-tight text-black dark:text-white uppercase font-mono">{product.ticker || product.nombre.substring(0,5)}</span>
          </div>
          <div className="flex items-center gap-6">
            <button onClick={() => onFavoriteToggle(product.id)} className={`text-[22px] ${isFavorite ? 'text-green-500' : 'text-black dark:text-white'}`}>
              <i className="fa-solid fa-cart-shopping"></i>
            </button>
            <button className="text-[20px] text-black dark:text-white" onClick={() => navigator.share({ title: 'TradingChango', text: `Precio de ${product.nombre}`, url: window.location.href })}>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar bg-white dark:bg-black">
          {/* Info Section - Left Image, Right Text */}
          <div className="flex gap-6 items-start mb-8">
            <div className="w-[120px] h-[120px] bg-white rounded-2xl border border-slate-100 dark:border-[#2a2a2a] flex-shrink-0 flex items-center justify-center p-3 overflow-hidden shadow-sm">
              <img 
            ...
            <div className="flex-1">
              <h1 className="text-[26px] font-[800] text-black dark:text-white leading-[1.1] tracking-tighter mb-1">{product.nombre}</h1>
              <div className="text-[10px] font-[800] text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2.5">
                MEJOR PRECIO HOY EN {bestStoreName}
              </div>
              <div className="text-[44px] font-mono font-[700] tracking-tighter text-black dark:text-white leading-none">
                $ {format(minPrice)}
              </div>
              <div className="mt-4">
                <div className="bg-[#f1f3f6] dark:bg-[#1e222d] border border-slate-100 dark:border-[#2a2e39] rounded-lg px-4 py-1.5 inline-flex items-center gap-2">
                  <span className="text-[11px] font-[700] text-slate-500 dark:text-[#b2b5be] tracking-tight">Precio promedio:</span>
          ...
          {/* Range Selection - EXACT AS SCREENSHOT */}
          <div className="flex gap-1.5 mb-10 justify-between">
            {[7, 15, 30, 90, 180, 365].map(d => (
              <button 
                key={d} 
                onClick={() => setDays(d)} 
                className={`flex-1 px-1 py-2.5 text-[10px] font-[800] rounded-lg border transition-all ${days === d ? 'bg-[#131722] dark:bg-white text-white dark:text-black border-[#131722] dark:border-white' : 'bg-white dark:bg-[#1e222d] text-slate-400 dark:text-[#b2b5be] border-slate-200 dark:border-[#363a45]'}`}
              >
          ...
          {/* Chart Section - EXACT AS SCREENSHOT */}
          <div className="mb-12">
            <h3 className="text-[11px] font-[800] uppercase tracking-[0.2em] text-black dark:text-white mb-1.5">GRÁFICO DE TENDENCIAS</h3>
            <div className={`text-[13px] font-[800] mb-6 ${trendColor === "#f23645" ? 'text-[#f23645]' : 'text-[#00c853]'}`}>
              {variation ? `${parseFloat(variation) > 0 ? '+' : ''}${variation}% últimos días` : '- 0.0%'}
            </div>
            
            <div className="h-72 w-full relative">
              {loading ? (
                <div className="h-full flex items-center justify-center font-mono text-[10px] text-slate-400 animate-pulse uppercase tracking-widest">Calculando tendencia...</div>
              ) : chartData.length > 1 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fontSize: 9, fill: '#94a3b8', fontWeight: 'bold'}} minTickGap={40} hide={false} />
          ...
          {/* Market Comparison - EXACT AS SCREENSHOT */}
          <section className="pb-24">
            <h3 className="text-[11px] font-[800] uppercase tracking-[0.2em] text-black dark:text-white mb-8">COMPARACION DE MERCADO</h3>
            <div className="space-y-6">
              {STORES.map((s) => {
                ...
                return (
                  <div key={s.name} className="flex items-center justify-between border-b border-dotted border-slate-100 dark:border-[#2a2a2a] pb-5 last:border-0">
                    <div className="flex items-center gap-3">
                      <div className="w-2.5 h-2.5 rounded-full" style={{backgroundColor: s.color}}></div>
                      <span className="text-[15px] font-[800] text-black dark:text-white uppercase tracking-tight">{s.name}</span>
                      {of && (
                        <span className="bg-[#00c853] text-white text-[9px] font-[800] px-1.5 py-0.5 rounded-sm uppercase ml-1">
                    ...
                    {productUrl ? (
                      <a 
                        ...
                        className={`font-mono text-[17px] font-[700] hover:opacity-70 transition-all ${isBest ? 'text-[#00c853]' : 'text-black dark:text-white'}`}
                      >
                        ${format(price)}
                      </a>
                    ) : (
